...
-- LENGTH(이름)로 길이 계산
SELECT  PROFESSOR_NO AS `교수번호`, PROFESSOR_NAME AS `교수명`, 
substring(PROFESSOR_NAME, 1,1) AS `성`, CHAR_length(PROFESSOR_NAME) AS `이름길이`
FROM PROFESSOR;
...
SELECT STUDENT_NO AS `학번`, STUDENT_NAME AS `이름`,  
substring('남여남여', substring(STUDENT_SSN, locate('-', STUDENT_SSN)+1,1) , 1) AS `성별(남/여)`
FROM STUDENT;
...
SELECT DEPARTMENT_NO AS `학과번호`, COUNT(*) AS `학과별학생수`
FROM STUDENT
GROUP BY `학과번호`
...
SELECT MAX(point) AS `최고점`, MIN(point) AS `최저점`, ROUND(AVG(point), 2)  AS `평균점수(소수점 2자리까지)`
FROM GRADE;
...
GROUP BY DEPARTMENT_NO, ENTRANCE_DATE;
...
SELECT COUNT(DISTINCT (CLASS_NO)) AS `전체 교수가 담당하는 서로 다른 과목의 수`
...
SELECT PROFESSOR_NO AS `교수번호`, COUNT(*) AS `지도학생수`
FROM PROFESSOR
GROUP BY PROFESSOR_
ORDER BY `지도학생수` DESC;
