...
discount_rate DECIMAL(5,2) DEFAULT 0.00 CHECK(discount_rate<=100 AND discount_rate>=0),
...
*/
INSERT CATEGORY
VALUES (NULL,'전자제품', '스마트폰, 노트북, 태블릿 등', TRUE, DEFAULT),
(NULL,'의류', '남성복, 여성복, 아동복', TRUE, DEFAULT),
(NULL,'도서', '소설, 전문서적, 교육서적', TRUE, DEFAULT),
(NULL,'스포츠/레저', '운동용품, 아웃도어 장비', TRUE, DEFAULT),
(NULL,'식품', '신선식품, 가공식품', FALSE, DEFAULT);
...
INSERT CUSTOMER
VALUES (NULL,'hong123', 'hong@email.com', 'password123', '홍길동', '010-1111-2222', '1990-05-15', 'M', 5000, 'GOLD',DEFAULT,DEFAULT,NULL),
(NULL,'kim_user', 'kim@email.com', 'mypass456', '김영희', '010-3333-4444', '1995-08-20', 'F', 3000, 'SILVER',DEFAULT,DEFAULT,NULL),
(NULL,'park2024', 'park@email.com', 'secure789', '박민수', '010-5555-6666', '1988-12-03', 'M', 10000, 'PLATINUM',DEFAULT,DEFAULT,NULL),
(NULL,'lee_shop', 'lee@email.com,', 'password999', '이수진', '010-7777-8888', '2000-03-10', 'F', 1500, 'BRONZE',DEFAULT,DEFAULT,NULL),
(NULL,'choi_buy', 'choi@email.com', 'pass1234', '최준호', NULL, '1992-07-25', 'M', 0, 'BRONZE',DEFAULT,DEFAULT,NULL);
...
INSERT PRODUCT
VALUES (NULL,'iPhone 15 Pro', 1, 1200000, 5.00, 50, '최신 아이폰 모델', 'Apple', 200.00, 'AVAILABLE',DEFAULT,DEFAULT),
(NULL,'삼성 갤럭시 북', 1, 1500000, 10.00, 30, '고성능 노트북', 'Samsung', 1500.00, 'AVAILABLE',DEFAULT,DEFAULT),
(NULL,'남성 정장', 2, 200000, 20.00, 100, '비즈니스 정장', 'Hugo Boss', NULL, 'AVAILABLE',DEFAULT,DEFAULT),
(NULL,'운동화', 4, 150000, 15.00, 0, '러닝화', 'Nike', 300.00, 'OUT_OF_STOCK' ,DEFAULT,DEFAULT),
(NULL,'요리책', 3, 25000, 0.00, 200, '집밥 요리 레시피', '맛있는책', 500.00, 'AVAILABLE',DEFAULT,DEFAULT);
...
CREATE OR REPLACE VIEW V_VIP_CUSTOMERS AS -- ~처럼 보이게 V_VIP_CUSTOMERS VIEW 생성
...
SELECT c.category_name, COUNT(p.product_id) AS product_count, AVG(p.price) avg_price
FROM PRODUCT p 
...
왼쪽 테이블을 조건 관계없이 모든 데이터를 보여주기
오른쪽 테이블은 일치하는 데이터가 없으면 해당 컬럼은 NULL표시

결과적으로는 상품이 존재하지 않는 식품 카테고리도 목록에 나타나고 데이터는 NULL 표시
LEFT 왼쪽 테이블은 FROM에 작성한 테이블
