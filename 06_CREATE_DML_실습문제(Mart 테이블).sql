...
-- mart_id, mart_name,  location,  phone,  open_time, close_time, is_24hour, created_at , updated_at
-- 1. 존재하지 않는다면 추가 
-- CREATE TABLE IF NOT EXISTS MART;
-- 2. 기존테이블이 존재할 경우 삭제 후 다시생성
-- DROP TABLE IF EXISTS MART;
CREATE TABLE IF NOT EXISTS MART (
    mart_id INT AUTO_INCREMENT PRIMARY KEY,
    mart_name VARCHAR(100) NOT NULL,
    location VARCHAR(255) NOT NULL,
    phone VARCHAR(20), -- INT 는 010으로 작성시 맨앞의 0 자동 삭제
    open_time TIME,
    close_time TIME,
...
INSERT INTO MART
VALUES (NULL,'CU 편의점 역삼점', '서울시 강남구 역삼로 789', '02-789-0123',NULL,NULL,TRUE,DEFAULT,DEFAULT);
-- AUTO_INCREMENT는 DEFAULT 미작성 상관 없이 자동으로 번호순번매김
...
-- -> 현재 시간과 10분이라는 시간 간격의 단위
-- DATE_SUB() 
-- -> 날짜에서 지정한 간격을 빼는 함수
-- 위 코드를 이용해서 등록된지 10분 이내 마트만 조회하기
SELECT * FROM MART
WHERE created_at >= DATE_SUB(NOW(), INTERVAL 10 MINUTE);

-- 전화번호, 핸드폰번호, 주민등록번호에는 varchar사용
-- ex) 010-..., 000102-.. 의 경우 INT라면 맨앞의 0을 제외해버림
...
VALUES(NULL,'김철수','IT','팀장','5000000','2023-01-15','kim.cs@company.com','010-1111-2222',DEFAULT,DEFAULT,DEFAULT),
...
VALUES(NULL,'웹사이트 리뉴얼','1','50000000','2024-01-01','2024-06-30','IN_PROGRESS','',DEFAULT),
(NULL,'모바일 앱 개발','3','80000000','2024-02-15','2024-12-31','PLANNING','',DEFAULT);
...
-- ========================================
-- 3. INSERT 서브쿼리
-- MySQL에서 같은 테이블을 insert 하면서 동시에 select를 하려고 할 때 발생하는 문제
...
VALUES('홍길동', 'IT', '사원', '2024-09-01','hong.gd@company.com',(SELECT AVG(salary) FROM (SELECT salary FROM employees WHERE department='IT') AS t));
...
FROM employees 
WHERE salary=(SELECT MAX(salary) FROM employees WHERE department='IT') AND department='IT');

-- SET해서 변수 명칭으로 사용하는 것은 INSERT와 UPDATE에서 주로 사용
-- SET 변수는 데이터 1개의 값만 저장 가능

INSERT INTO projects
VALUES(NULL,'AI 챗봇 개발',@직원아이디,'120000000','2024-10-01','2025-03-31','PLANNING','',DEFAULT);
...
INSERT INTO employees (emp_name, department, position, hire_date, email,salary)
VALUES('윤서연', 'HR', '사원',@마지막입사자,'yoon.sy@company.com','3800000');
...
SET @전체평균급여 = (SELECT AVG(salary) FROM employees);
UPDATE employees
SET salary = @전체평균급여
...
SET @상태매니저 = (SELECT manager_id FROM projects WHERE status ='COMPLETED');
...
DELETE FROM
employees 
WHERE salary <3000000 AND hire_date < '2024-01-01';
